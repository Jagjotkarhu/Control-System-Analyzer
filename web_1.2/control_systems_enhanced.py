...
import plotly.graph_objects as go # type: ignore # type : ignore[import-not-found]
from plotly.subplots import make_subplots # type: ignore # type : ignore[import-not-found]
from scipy.linalg import expm #type: ignore[import-not-found]
from scipy.optimize import fminbound # type: ignore[import-not-found]

# --- 1. CORE ANALYSIS CLASS (ENHANCED) ---
class TFAnalyzer:
    ...
    def time_domain_metrics(self):
        ...
        """
        t, y = ct.step_response(self.sys, T=np.linspace(0, 10, 2000))
...
try:
    ...
    if show_simple:
        ...
        mag_fd, phase_fd, omega_fd, _, _ = analyzer.freq_domain_metrics().values()
        ...
        with plot_col2:
            ...
            fig_bode.add_trace(
                go.Scatter(x=omega_fd, y=20*np.log10(np.abs(mag_fd) + 1e-10), 
                          line=dict(color='#ff7f0e', width=2),
                          name='Magnitude'),
                row=1, col=1
            )
            fig_bode.add_trace(
                go.Scatter(x=omega_fd, y=np.degrees(phase_fd),
                          line=dict(color='#2ca02c', width=2),
                          name='Phase'),
                row=2, col=1
            )
            fig_bode.update_xaxes(type="log", row=2, col=1)
            fig_bode.update_yaxes(title_text="Mag (dB)", row=1, col=1)
    ...
    if show_metrics:
        ...
        with tab1:
            if analysis_domain in ["Time Domain", "Both"]:
                ...
                with col_td1:
                    st.metric("Rise Time (tᵣ)", f"{td_data['rise_time']:.4f} s", 
                             help="10% to 90% settling time")
                with col_td2:
                    st.metric("Peak Time (tₚ)", f"{td_data['peak_time']:.4f} s",
                             help="Time at maximum output")
                with col_td3:
                    st.metric("Overshoot (Mₚ)", f"{td_data['overshoot']:.2f}%",
                             help="(Peak - Final) / Final * 100")
                with col_td4:
                    st.metric("Settling Time (tₛ)", f"{td_data['settling_time']:.4f} s",
                             help="2% criterion: 4/(ζ·ωₙ)")
                
                # Step response with annotations
                fig_step_annot = go.Figure()
                fig_step_annot.add_trace(go.Scatter(
                    x=td_data['step_t'], y=td_data['step_y'],
                    mode='lines', line=dict(color='#1f77b4', width=2.5),
                    name='Step Response'
                ))
                
                # Add horizontal line at steady state
                fig_step_annot.add_hline(y=td_data['final_value'], 
                                        line_dash="dash", line_color="gray",
                                        annotation_text=f"SS: {td_data['final_value']:.3f}")
                
                # Add horizontal line at peak
                fig_step_annot.add_hline(y=td_data['peak_value'], 
                                        line_dash="dot", line_color="red",
                                        annotation_text=f"Peak: {td_data['peak_value']:.3f}")
                
                fig_step_annot.update_layout(
                    title="Step Response with Annotations",
                    xaxis_title="Time (s)",
                    yaxis_title="Output",
                    template="plotly_white",
                    hovermode='x unified'
                ...
                st.info("Select 'Time Domain' or 'Both' to view time domain analysis")
        ...
        with tab2:
            if analysis_domain in ["Frequency Domain", "Both"]:
                ...
                with col_fd1:
                    gm_val = fd_data['gain_margin'] if fd_data['gain_margin'] != np.inf else float('inf')
                    st.metric("Gain Margin", f"{gm_val:.2f} dB" if gm_val != np.inf else "∞",
                             help="Stability margin at phase = -180°")
                
                with col_fd2:
                    st.metric("Phase Margin", f"{fd_data['phase_margin']:.2f}°",
                             help="Stability margin at gain = 0 dB")
                
                with col_fd3:
                    st.metric("Bandwidth", f"{fd_data['bandwidth']:.3f} rad/s",
                             help="Frequency at -3dB from DC gain")
                
                with col_fd4:
                    st.metric("DC Gain", f"{20*np.log10(fd_data['dc_gain']):.2f} dB",
                             help="Magnitude at ω = 0")
                
                # Detailed Bode
                fig_bode_detail = make_subplots(
                    rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.12,
                    subplot_titles=("Magnitude Response", "Phase Response")
                )
                
                fig_bode_detail.add_trace(
                    go.Scatter(x=fd_data['omega'], 
                              y=20*np.log10(np.abs(fd_data['mag']) + 1e-10),
                ...
                # Add margin markers
                if fd_data['wgc'] > 0:
                    fig_bode_detail.add_vline(x=fd_data['wgc'], line_dash="dash", 
                                             line_color="red", row=1, col=1,
                                             annotation_text="Gain Crossover")
                if fd_data['wpc'] > 0:
                    fig_bode_detail.add_vline(x=fd_data['wpc'], line_dash="dash",
                                             line_color="purple", row=2, col=1,
                                             annotation_text="Phase Crossover")
                
                fig_bode_detail.update_xaxes(type="log", row=2, col=1)
                fig_bode_detail.update_yaxes(title_text="Magnitude (dB)", row=1, col=1)
                fig_bode_detail.update_yaxes(title_text="Phase (degrees)", row=2, col=1)
                fig_bode_detail.update_xaxes(title_text="Frequency (rad/s)", row=2, col=1)
                fig_bode_detail.update_layout(
                    title="Detailed Bode Plot",
                    template="plotly_white",
                    showlegend=False,
                    hovermode='x unified'
                )
                ...
                with st.expander("📐 Formulas Used"):
                    ...
                    **Frequency Domain Calculations:**
                    - **Gain Margin (GM)**: Margin at phase = -180°
                    - **Phase Margin (PM)**: Margin at magnitude = 0 dB
                    - **Bandwidth**: Frequency where magnitude drops 3dB from DC
                    - **Crossover Frequencies**: wgc (gain), wpc (phase)
                    - **Stability**: GM > 0 dB AND PM > 0° → Stable System
                    """)
            else:
                st.info("Select 'Frequency Domain' or 'Both' to view frequency domain analysis")
        
        # --- COMBINED TAB (if Both selected) ---
        if analysis_domain == "Both":
            with tab3:
                ...
                with col_rel1:
                    ...
                    **Time Domain → Frequency Domain:**
                    - Rise Time ↔ Bandwidth (inverse relationship)
                    - Overshoot ↔ Phase Margin (ζ inversely related to PM)
                    - Settling Time ↔ Gain Margin robustness
                    
                    **Design Trade-offs:**
                    - Higher BW → Faster response, more noise
                    - Higher PM → More stable, slower response
                    """)
    ...
    if show_poles:
        ...
        if len(poles) > 0:
            fig_pz.add_trace(go.Scatter(
                ...
                name='Poles', text=[f'Pole {i+1}' for i in range(len(poles))],
                hovertemplate='%{text}<br>σ=%{x:.3f}<br>ω=%{y:.3f}<extra></extra>'
            ))
        ...
        if len(zeros) > 0:
            fig_pz.add_trace(go.Scatter(
                ...
                mode='markers', marker=dict(size=12, color='blue', symbol='o'),
                name='Zeros', text=[f'Zero {i+1}' for i in range(len(zeros))],
                hovertemplate='%{text}<br>σ=%{x:.3f}<br>ω=%{y:.3f}<extra></extra>'
            ))
        
        # Add stability boundary (jω axis)
        fig_pz.add_vline(x=0, line_dash="dash", line_color="gray", 
                        annotation_text="Stability Boundary (jω axis)")
        
        # Add regions for reference
        fig_pz.add_vrect(x0=-5, x1=0, fillcolor="green", opacity=0.1, layer="below",
                        annotation_text="Stable Region", annotation_position="top left")
        
        fig_pz.update_layout(
            title="Pole-Zero Map",
            xaxis_title="Real (σ)",
            yaxis_title="Imaginary (ω)",
            template="plotly_white",
            hovermode='closest',
        ...
        for i, pole in enumerate(poles):
            ...
            pole_data.append({
                'Pole #': i+1,
                'Real (σ)': f"{real_part:.4f}",
                'Imaginary (ω)': f"{imag_part:.4f}",
                'Magnitude': f"{magnitude:.4f}",
                'Angle (°)': f"{angle:.2f}"
            })
        
        st.dataframe(pole_data, use_container_width=True)
    ...
    if show_ctrl:
        ...
        is_controllable = rank_Cm == sys_order
        
        col_ctrl1, col_ctrl2 = st.columns(2)
        with col_ctrl1:
            st.metric("System Order", sys_order)
        with col_ctrl2:
            st.metric("Controllability", 
                     "✅ Fully Controllable" if is_controllable else "❌ Uncontrollable",
                     delta=f"Rank={rank_Cm}/{sys_order}")
        
        # Display matrices
        st.subheader("State Space Matrices")
        col_a, col_b = st.columns(2)
        with col_a:
            st.write("**A Matrix (State):**")
            st.dataframe(np.round(A, 4), use_container_width=True)
        with col_b:
            st.write("**B Matrix (Input):**")
            st.dataframe(np.round(B, 4), use_container_width=True)
        
        st.write("**Controllability Matrix Cₘ = [B AB A²B ...]:**")
        st.dataframe(np.round(Cm, 4), use_container_width=True)
        
        # Pole placement
        if is_controllable:
            st.subheader("Design State Feedback Gain K")
            ...
            try:
                ...
                if len(desired_poles) == sys_order:
                    ...
                    st.write(f"**Desired Poles:** {desired_poles}")
                    ...
                    st.write("**Closed-Loop Poles (verification):**")
                    st.dataframe(np.round(poles_cl, 4), use_container_width=True)
                else:
                    st.warning(f"⚠️ Please provide exactly {sys_order} poles")
            except Exception as e:
                st.error(f"❌ Error parsing poles. Use format: 'a+bj, c+dj' \nError: {e}")
        else:
            st.error("❌ System is uncontrollable. Cannot place poles.")
    ...
    if show_stm:
        ...
        with st.expander("📖 Understanding Φ(t)"):
            st.markdown(f"""
            **State Transition Matrix Φ(t) = e^(At)**
            
            Describes how system state evolves:
            **x(t) = Φ(t)·x(0) + ∫Φ(t-τ)B·u(τ)dτ**
            
            At t={t_val}s:
            - Shows state evolution from t=0 to t={t_val}
            - Eigenvalues of A determine stability
            - Used for predicting future state given initial conditions
            """)
    ...
    if show_comp:
        ...
        T = ct.feedback(L, 1)  # Closed-loop with unity feedback
        
        # Get metrics for compensated system
        mag_L, phase_L, omega_L, gm_L, pm_L = analyzer.freq_domain_metrics()
        mag_comp, phase_comp, omega_comp, gm_comp, pm_comp_val = analyzer.freq_domain_metrics()
        
        try:
            gm_comp, pm_comp_val, _, _ = ct.margin(L)
        except:
            gm_comp, pm_comp_val = np.inf, 0
        
        # Get step response
        t_comp, y_comp = ct.step_response(T, T=np.linspace(0, 5, 500))
        
        st.info(f"""
        **Current Compensator:** Gc(s) = {K_val:.2f} · (s + {z_val:.2f}) / (s + {p_val:.2f})
        """)
        
        # Metrics
        col_comp1, col_comp2, col_comp3 = st.columns(3)
        with col_comp1:
            st.metric("New PM", f"{pm_comp_val:.2f}°" if not np.isnan(pm_comp_val) else "N/A")
        with col_comp2:
            gm_db = 20*np.log10(gm_comp) if gm_comp != np.inf and gm_comp > 0 else np.inf
            st.metric("New GM", f"{gm_db:.2f} dB" if gm_db != np.inf else "∞")
        with col_comp3:
            st.metric("Comp Type", comp_type)
        ...
        with plot_col1:
            ...
            try:
                t_orig, y_orig = ct.step_response(ct.feedback(analyzer.sys, 1), 
                                                 T=np.linspace(0, 5, 500))
                fig_comp_step.add_trace(go.Scatter(
                    x=t_orig, y=y_orig, mode='lines',
            ...
            fig_comp_step.add_trace(go.Scatter(
                x=t_comp, y=y_comp, mode='lines',
                line=dict(color='purple', width=2.5),
                name='Compensated'
            ))
            
            fig_comp_step.update_layout(
        ...
        with plot_col2:
            fig_comp_bode = make_subplots(
                rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.12,
                subplot_titles=("Open-Loop Magnitude", "Open-Loop Phase")
            )
            
            # Original
            try:
                mag_orig, phase_orig, omega_orig, _, _ = analyzer.freq_domain_metrics()
                fig_comp_bode.add_trace(
                    go.Scatter(x=omega_orig, y=20*np.log10(np.abs(mag_orig) + 1e-10),
                              line=dict(dash='dot', color='gray', width=1.5),
                              name='Orig Mag'),
                    row=1, col=1
                )
                fig_comp_bode.add_trace(
                    go.Scatter(x=omega_orig, y=np.degrees(phase_orig),
                              line=dict(dash='dot', color='gray', width=1.5),
                              name='Orig Phase'),
                    row=2, col=1
                )
            ...
            # Compensated
            fig_comp_bode.add_trace(
                go.Scatter(x=omega_comp, y=20*np.log10(np.abs(mag_comp) + 1e-10),
                          line=dict(color='orange', width=2.5),
                          name='Comp Mag'),
                row=1, col=1
            )
            fig_comp_bode.add_trace(
                go.Scatter(x=omega_comp, y=np.degrees(phase_comp),
            ...
            fig_comp_bode.update_layout(
                title="Open-Loop Bode Comparison",
                template="plotly_white",
                showlegend=False,
                hovermode='x unified'
            )
    ...
    st.info("Check input format: Numerator and Denominator as comma-separated coefficients")
...
---
**📚 Learning Path:**
- [ ] Study step response metrics
- [ ] Understand pole-zero maps
- [ ] Learn Bode plot interpretation
- [ ] Master compensator design
- [ ] Deploy to hardware (Arduino/PLC)

**🎯 Resume Ready:** All analyses are manual (no library shortcuts) → shows deep understanding!
""")
